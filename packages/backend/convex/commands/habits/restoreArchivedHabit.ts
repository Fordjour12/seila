...
export const restoreArchivedHabit = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const restoredHabitId = await ctx.db.insert("habits", {
      ...
      kind: archived.kind,
      breakGoal: archived.breakGoal,
      breakMetric: archived.breakMetric,
      targetValue: archived.targetValue,
      targetUnit: archived.targetUnit,
      targetType: archived.targetType,
      frequencyType: archived.frequencyType,
      frequencyConfig: archived.frequencyConfig,
      identityTags: archived.identityTags ?? [],
      energyLevel: archived.energyLevel ?? "low",
      timePreference: archived.timePreference ?? "flexible",
