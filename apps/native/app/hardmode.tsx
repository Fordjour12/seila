...
const MOCK_PLAN: PlannedItem[] = [
  ...
  },
  {
    ...
    rationale:
      "Walk days show 0.8pt mood increase in your data. Scheduled early to match your pattern.",
    confidence: "high",
  },
  {
    id: "3",
  ...
  },
  {
    ...
    rationale:
      "Evening anchor matches your stated preference. Lower effort to pair with end-of-day.",
    confidence: "medium",
  },
];
...
function ActivationFlow({
  ...
  onActivate: (scope: ModuleScope, duration: Duration) => void;
}) {
  ...
  return (
    <ScrollView
      style={actStyles.scroll}
      contentContainerStyle={actStyles.content}
    ...
    >
      ...
      <View style={actStyles.header}>
        ...
        <Text style={actStyles.subtitle}>
          You choose the scope and duration. The AI plans. You flag anything that doesn't fit. No
          streaks. No failure. Exit anytime.
        </Text>
      </View>
          ...
          {DURATIONS.map((d) => (
            ...
            >
              <View style={actStyles.durationLeft}>
                <Text
                  style={[
                    actStyles.durationLabel,
                    duration === d.id && actStyles.durationLabelActive,
                  ]}
                >
                  {d.label}
                </Text>
              ...
              <View
                style={[
                  actStyles.durationRadio,
                  duration === d.id && actStyles.durationRadioActive,
                ]}
              />
            </Pressable>
          ))}
      ...
      <View style={actStyles.note}>
        <Text style={actStyles.noteText}>
          Modules you don't select continue in normal mode. You can flag any AI decision at any
          time. Exiting is instant — no confirmation required.
        </Text>
      </View>
  ...
  return (
    <View style={flagStyles.overlay}>
      ...
      <Animated.View style={[flagStyles.sheet, { transform: [{ translateY: slideAnim }] }]}>
        ...
        <Text style={flagStyles.note}>No explanation needed. Flags are signal, not complaint.</Text>
      </Animated.View>
    </View>
  );
}
  ...
  return (
    ...
    >
      ...
      <View style={planStyles.mainCol}>
        ...
        {showRationale && <Text style={planStyles.rationale}>{item.rationale}</Text>}
        {item.flagged && (
          <Text style={planStyles.flaggedLabel}>{item.flagged.replace("_", " ")}</Text>
        )}
        {!showRationale && <Text style={planStyles.longPressHint}>hold to see why</Text>}
      </View>
      {!item.flagged && (
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: Spacing.xxl, paddingBottom: 40 }}>
        ...
        {plan.map((item) => (
          <PlanItemCard key={item.id} item={item} onFlag={() => setFlaggingItem(item)} />
        ))}

        {/* Crisis override */}
        <Pressable
          style={activeStyles.crisisBtn}
          onPress={() => {
            Alert.alert(
              "Clear today's plan",
              "This clears all planned items for today. No explanation needed.",
              [
