  ...
  return (
    <View className="absolute inset-0 bg-black/50 justify-end z-50">
      ...
      <View className="bg-surface rounded-t-3xl border-t border-border p-4 gap-2 pb-8">
        <Text className="text-lg font-medium text-foreground text-center pb-4">
          {title}
        </Text>
        ...
        <Pressable
          className="p-4 rounded-xl bg-background mt-2"
          onPress={onClose}
        >
...
export default function BudgetScreen() {
  ...
  const deleteEnvelope = useMutation(
    api.commands.accounts.setEnvelope.deleteEnvelope,
  );
  ...
  const [editingBudgetId, setEditingBudgetId] = React.useState<
    Id<"sharedBudgets"> | undefined
  >();
  ...
  const isLoading =
    envelopes === undefined || sharedBudgetOverview === undefined;
  ...
  const handleSharedSubmit = async () => {
    ...
    const spentAmountCents = spentAmount
      ? Math.round(parseFloat(spentAmount) * 100)
      : undefined;
    if (
      !name.trim() ||
      !Number.isInteger(budgetAmountCents) ||
      budgetAmountCents <= 0
    ) {
      toast.show({
        variant: "warning",
        label: "Enter valid shared budget details",
      });
      return;
    }
    if (
      spentAmountCents !== undefined &&
      (!Number.isInteger(spentAmountCents) || spentAmountCents < 0)
    ) {
  ...
  const handleDeleteSharedBudget = async () => {
    ...
    try {
      ...
      toast.show({
        variant: "danger",
        label: "Failed to delete shared budget",
      });
    }
  };
      ...
      return [
        {
          label: "Edit Envelope",
          onPress: () =>
            router.push(`/(tabs)/finance/budget/edit?id=${longPressTarget.id}`),
        },
        {
  ...
  return (
    ...
    >
          ...
          <View className="flex-1 gap-1">
            ...
            <Text
              className={`text-xl font-medium ${
                combinedBudget - combinedSpent >= 0
                  ? "text-success"
                  : "text-danger"
              }`}
            >
        ...
        {showSharedForm && (
          <View className="bg-surface rounded-2xl border border-border p-4 gap-3 shadow-sm">
            ...
            <View className="flex-row gap-2">
              <Button
                label={
                  isSubmitting ? "Saving..." : editingBudgetId ? "Save" : "Add"
                }
            ...
            (sharedBudgetOverview?.items || []).map((item) => (
              ...
              >
                ...
                <Text className="text-xs text-muted-foreground">
                  Members:{" "}
                  {item.members.length ? item.members.join(", ") : "None"}
                </Text>
              </Pressable>
            ))
          )}
      ...
      <ActionSheet
        ...
        title={
          longPressTarget ? `What to do with "${longPressTarget.name}"?` : ""
        }
