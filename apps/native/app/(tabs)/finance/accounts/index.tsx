...
export default function FinanceAccountsScreen() {
  ...
  const [isArchivingAccountId, setIsArchivingAccountId] = React.useState<
    string | null
  >(null);
    ...
    >
      <View className="mb-2">
        ...
        <Text className="text-sm text-muted-foreground mt-1">
          Track balances, runway, and savings progress.
        </Text>
      </View>
        ...
        <>
          <View className="gap-3">
            ...
            <View className="bg-surface rounded-2xl border border-border p-4 gap-3 shadow-sm">
              <View className="flex-row justify-between items-center">
                <View>
                  <Text className="text-xs text-muted-foreground uppercase tracking-wider font-medium mb-1">
                    Monthly Close
                  </Text>
              ...
              <View className="flex-row justify-between items-center">
                <Text className="text-sm text-muted-foreground">
                  In {formatGhs(cashflow?.monthIncome || 0)} · Out{" "}
              ...
              <View className="flex-row flex-wrap gap-2 mt-2">
                ...
                >
                  <Text
                    className={`text-xs font-medium ${showIncomeInput ? "text-warning" : "text-foreground"}`}
                  >
                    Custom
                  </Text>
                </Pressable>
                  ...
                  <Pressable
                    ...
                    onPress={() => {
                      const amount = Math.round(
                        parseFloat(customIncomeAmount) * 100,
                      );
                      if (amount > 0) {
          ...
          <View className="gap-3">
            <View className="flex-row justify-between items-center">
              ...
              <Pressable
                onPress={() =>
                  router.push("/(tabs)/finance/accounts/add" as any)
                }
              >
              ...
              {(goals || []).length === 0 ? (
                ...
                <View className="gap-4">
                  {(goals || []).slice(0, 3).map((goal, index) => (
                    <View
                      key={goal.goalId}
                      className={`${index > 0 ? "border-t border-border pt-4" : ""} gap-2`}
                    >
                      <View className="flex-row justify-between items-center">
                      ...
                      <View className="h-1.5 bg-background rounded-full overflow-hidden">
                        <View
                          ...
                          style={{
                            width: `${Math.min(100, Math.max(0, (goal.progress || 0) * 100))}%`,
                          }}
                        />
                      </View>
                      <View className="flex-row flex-wrap gap-2">
                        {[1000, 5000].map((amount) => (
                          <Pressable
                            ...
                            onPress={() =>
                              handleContributeGoal(goal.goalId, amount)
                            }
                          >
                            <Text className="text-xs text-foreground font-medium">
                              +{formatGhs(amount)}
                  ...
                  {(goals || []).length > 3 && (
                    <Pressable
                      onPress={() => router.push("/(tabs)/finance/savings")}
                    >
          ...
          <View className="gap-3">
            ...
            <View className="flex-row flex-wrap gap-3">
              {[
                ...
                <Pressable
                  key={nav.key}
                  className={`${index === items.length - 1 && items.length % 2 === 1 ? "w-full" : "w-[48%]"} min-h-32 bg-surface rounded-2xl border border-border p-4 gap-2 active:bg-muted/10`}
