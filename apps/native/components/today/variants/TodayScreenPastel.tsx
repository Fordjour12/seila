...
function CheckinCard({ onPress }: { onPress: () => void }) {
   ...
   return (
      <Pressable
         onPress={onPress}
         style={({ pressed }) => [chk.card, pressed && chk.cardPressed]}
      >
         {/* Header */}
         ...
         <View style={chk.readouts}>
            ...
            <View style={[chk.readoutBlob, { backgroundColor: P.peachSoft }]}>
               ...
               <Text style={[PT.eyebrow, { color: P.peachDeep, marginTop: 4 }]}>
                  energy
               </Text>
               <View style={chk.scale}>
                  {[1, 2, 3, 4, 5].map((n) => (
         ...
         <View style={chk.footer}>
            <Text style={[PT.label, { color: P.lavenderDeep }]}>
               Check in again →
            </Text>
         </View>
      </Pressable>
...
function NextActionCard({
   ...
   return (
      <Pressable
         onPress={onPress}
         style={({ pressed }) => [na.card, pressed && { opacity: 0.92 }]}
      >
...
function HabitPill({
   ...
   return (
      <Pressable
         onPress={onToggle}
         style={[hp.pill, habit.done && hp.pillDone]}
      >
         {/* Emoji bubble */}
...
function FocusTaskPill({
   ...
   return (
      <Pressable
         onPress={onComplete}
         style={({ pressed }) => [ft.pill, pressed && { opacity: 0.8 }]}
      >
         <View style={ft.dot} />
      ...
      <SafeAreaView style={s.safe}>
         ...
         >
            ...
            <View style={s.section}>
               <SoftSectionHeader
                  label="Habits"
                  meta={`${doneCount} of ${habits.length}`}
               />
            ...
            <View style={s.section}>
               ...
               {focus.length === 0 ? (
                  <View style={s.emptyPill}>
                     <Text style={[PT.bodyLight, { textAlign: "center" }]}>
                        All clear ✦
                     </Text>
                  </View>
               ) : (
                  <View style={s.pillStack}>
                     {focus.map((task) => (
                        <FocusTaskPill
                           ...
                           task={task}
                           onComplete={() =>
                              setFocus((prev) => prev.filter((t) => t.id !== task.id))
                           }
                        />
                     ))}
                  </View>
